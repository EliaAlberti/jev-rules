...
| 2 | Checkout bug, right rule | "the checkout total is wrong when a discount is applied, fix it..." | Payments rule 0.97, test conventions 0.73 and the checkout map 0.91 arrive with the prompt; ten rules stay out. Claude writes the tests first because the rule says so, then the one-line fix, shown as a diff. Each file change is judged live. | `checkout-3-decided`, `checkout-4-answer` |
